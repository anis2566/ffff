...
export const StudentAttendanceView = ({ id }: StudentAttendanceViewProps) => {
  const trpc = useTRPC();
  ...
  return (
    <div className="space-y-6">
      ...
      <ListCardWrapper title="Students" value={data?.attendances?.length}>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {data?.attendances?.map((attendance, index) => (
              <TableRow key={attendance.id}>
                ...
                <TableCell>
                    <Badge
                        variant={attendance.status === ATTENDANCE_STATUS.Present.toLocaleLowerCase() ? "default" : "destructive"}
