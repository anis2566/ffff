...
export const UtilityExpenseReportView = () => {
  const trpc = useTRPC();
  const [filters] = useGetSession();

  const { data } = useSuspenseQuery(
    trpc.report.utilityExpense.queryOptions({ ...filters })
  );

  const getTotalForMonth = (month: MONTH): number => {
    return (
      data?.utilities.reduce((total, item) => {
        ...
        <TableBody>
          {data?.utilities.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {item.months.reduce((acc, curr) => acc + curr.amount, 0)}
              </TableCell>
            </TableRow>
          ))}
