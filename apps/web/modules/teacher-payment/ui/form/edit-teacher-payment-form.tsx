...
interface EditTeacherPaymentProps {
  id: string;
}

export const EditTeacherPaymentFormView = ({ id }: EditTeacherPaymentProps) => {
  ...
  const { data: payment } = useSuspenseQuery(
    trpc.teacherPayment.getOne.queryOptions(id)
  );

  const { mutate: updatePayment, isPending } = useMutation(
    trpc.teacherPayment.updateOne.mutationOptions({
      onError: (err) => {
  ...
  const form = useForm<TeacherPaymentSchemaType>({
    ...
    defaultValues: {
      ...
      month: payment?.month || "",
      classUnit: payment?.classUnit?.toString() || "",
      incentive: payment?.incentive?.toString() || "",
      deductionUnit: payment?.deductionUnit?.toString() || "",
      note: payment?.note || "",
    },
  });
  ...
  const totalClassAmount =
    parseInt(form.watch("classUnit")) * (payment?.teacher?.classRate || 0);
  const totalIncentiveAmount = parseInt(form.watch("incentive") || "0");
  const totalDeductionAmount =
    parseInt(form.watch("deductionUnit") || "0") *
    (payment?.teacher?.classRate || 0);
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-gray-900 dark:text-white">
              {totalClass} * {payment?.teacher?.classRate} = {totalClassAmount}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-gray-900 dark:text-white">
              {totalDeduction} * {payment?.teacher?.classRate} ={" "}
